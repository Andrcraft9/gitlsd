...
- `App` is the central state machine. It owns preview visibility, focus, content, independent horizontal and vertical offsets for log, preview, and help, and separate preview search state alongside `Screen` and `InputMode`; the interactive frontend supplies the active pane width so horizontal movement is bounded and Home/End can target visible content.
